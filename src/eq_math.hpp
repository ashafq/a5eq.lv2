 ...
 */
static std::array<double, 5> compute_low_shelf(double fc, double gain, double q,
                                               double fs) {
  assert(fs > 0);
  assert(fc > 0);
  assert(q > 0);
  ...
  DBG(printf("%s: fc = %g, gain = %g, Q= %g, fs = %g\n", __func__, fc, gain, q,
             fs););
  DBG(printf("\t| w0 [%g], alpha [%g], A [%g]\n", w0, alpha, A););
  DBG(printf("\t| %g, %g, %g, %g, %g, %g\n", b0, b1, b2, a0, a1, a2););
 ...
 * @return std::array<double, 5> Coefficients in {b0, b1, b2, a1, a2}
 */
static std::array<double, 5> compute_high_shelf(double fc, double gain,
                                                double q, double fs) {
  assert(fs > 0);
  assert(fc > 0);
  assert(q > 0);
 ...
 * @return std::array<double, 5> Coefficients in {b0, b1, b2, a1, a2}
 */
static std::array<double, 5> compute_peaking(double fc, double gain, double q,
                                             double fs) {
  assert(fs > 0);
  assert(fc > 0);
  assert(q > 0);
